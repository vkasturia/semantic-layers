...
public class ProximityScore {
    public Object[] proximity_score_calc(String[] entityArray, String fromDate, String toDate, String timePeriod){
        ...
        for(Map.Entry<String, Map<String, List<Double>>> outerEntry: document_relatedEntity_averageDistanceList_map.entrySet()){
           String document = outerEntry.getKey();
            ...
            for (Map.Entry<String, List<Double>> innerEntry : outerEntry.getValue().entrySet()) {
                ...
                relatedEntityProximityScore /= entityArray.length;
        ...
        returnedValues[3] = relEntity_document_relEntityProximityScoreInDoc_map;
        ...
        for(Map.Entry<String, Map<String, List<Integer>>> outerEntry: document_queryEntity_positionList_map.entrySet()){
            ...
            for(Map.Entry<String, List<Integer>> innerEntry : queryEntity_positionList_map.entrySet()){
                ...
                for(Map.Entry<String, List<Integer>> innerEntry2: relatedEntity_positionList_map.entrySet()){
                    ...
                    for (Integer relatedEntityEntry : relatedEntityPositionList) {
                        double absoluteDifferenceSum = 0;
                        for (Integer queryEntityEntry : queryEntityPositionList) {
                            double difference = Math.abs(relatedEntityEntry - queryEntityEntry);
                            absoluteDifferenceSum += difference;
                        }
                        absoluteDifferenceSum /= queryEntityPositionList.size();
                        averageDistance += absoluteDifferenceSum;
                    }
                    averageDistance /= relatedEntityPositionList.size();
    ...
    public Object[] entity_position(String[] entityArray, String fromDate, String toDate, String timePeriod) {
        ...
        while (counter > 9999) {
            ...
            for (int i = 1; i < entityArray.length; i++) {
                ParameterizedSparqlString EntityPositionCounter_toAppend = new ParameterizedSparqlString("?article schema:mentions ?entity"+i+" .\n" +
                                                                                                         "?entity"+i+" oae:hasMatchedURI <"+ entityArray[i] +">. \n");
                EntityPositionCounter.append(EntityPositionCounter_toAppend);
            }

            ParameterizedSparqlString EntityPositionCounter_endAppend = new ParameterizedSparqlString("}LIMIT 10000 OFFSET " + h);

            EntityPositionCounter.append(EntityPositionCounter_endAppend);
            ...
            for (int i = 1; i < entityArray.length; i++) {
                ParameterizedSparqlString PositionCountQuery_toAppend = new ParameterizedSparqlString("?article schema:mentions ?entity"+i+" .\n" +
                                                                                                      "?entity"+i+" oae:hasMatchedURI <"+ entityArray[i] +">. \n");
                PositionCountQuery.append(PositionCountQuery_toAppend);
            }

            ParameterizedSparqlString PositionCountQuery_endAppend = new ParameterizedSparqlString("}LIMIT 10000 OFFSET " + h);

            PositionCountQuery.append(PositionCountQuery_endAppend);
