...
import ranking_algo_or.VirtuosoConnector;
...
public class RelatednessScore {
    public Map<String, Double> relatedness_score(String[] entityArray, String fromDate, String toDate, String timePeriod, int counter, Map<String, Map<String, Double>> relEntity_timePeriod_relEntityProximityScoreInPeriod_map, Map<String, Map<String, Double>> relEntity_document_relEntityProximityScoreInDoc_map) throws FileNotFoundException{
        ...
        for (Map.Entry<String, List<String>> entry : articleUrl_relEntities_Map.entrySet()) {
            ...
            for (String relentity : relEntities) {
                ...
                for (Map.Entry<String, Map<String, List<String>>> timeperiod : timePeriod_relEntities_Map.entrySet()) {
                    String timeperiodToString = timeperiod.getKey();
                    ...
                    if (timeperiod.getValue().get(relentity) == null) {
                        ...
                        entity_subrelatednessScore *= relEntity_eoiPercentageScore_map.get(relentity);
                        //entity_subrelatednessScore *= relEntity_timePeriod_relEntityProximityScoreInPeriod_map.get(relentity).get(timeperiodToString);
                    }
                    entity_relatednessScore += entity_subrelatednessScore;
                    }catch(NullPointerException e){System.out.println(relentity +" : " + timeperiodToString);}
                }
                double idfValue = article_idfscore_map.get(relentity);
                entity_relatednessScore *= (idfValue * relEntity_document_relEntityProximityScoreInDoc_map.get(relentity).get(articleUrl));
    ...
    public Object[] entitiesreturn(String[] entityArray, String fromDate, String toDate, String timePeriod) {
        ...
        while (counter > 9999) {
            ...
            ParameterizedSparqlString EntitiesCounter = new ParameterizedSparqlString("PREFIX oa: <http://www.w3.org/ns/oa#>\n" +
                                                                                      ...
                                                                                      "SELECT COUNT DISTINCT ?article  ?entityURI2 ?"+timePeriod+"\n" +
                                                                                      "WHERE {\n" +
            ...
            ParameterizedSparqlString EntitiesCounter_toAppend3 = new ParameterizedSparqlString("}LIMIT 10000 OFFSET " + h);
            ...
            ParameterizedSparqlString Docs_and_Entities = new ParameterizedSparqlString("PREFIX oa: <http://www.w3.org/ns/oa#>\n" +
                                                                                        ...
                                                                                        "PREFIX xsd: <http://www.w3.org/2001/XMLSchema#>\n" +
                                                                                        "PREFIX schema:  <http://schema.org/>\n" +
                                                                                        "SELECT DISTINCT ?article  ?entityURI2 ?"+timePeriod+"\n" +
                                                                                        "WHERE {\n" +
                                                                                        "?article dc:date ?date FILTER(?date >= xsd:date('"+fromDate+"') &&\n" +
                                                                                        "?date <= xsd:date('"+ toDate +"')).\n" +
                                                                                        "BIND (CONCAT(month(?date), \"-\", year(?date)) AS ?month). \n" +
            ...
            ParameterizedSparqlString Docs_and_Entities_toAppend3 = new ParameterizedSparqlString("}LIMIT 10000 OFFSET " + h);
