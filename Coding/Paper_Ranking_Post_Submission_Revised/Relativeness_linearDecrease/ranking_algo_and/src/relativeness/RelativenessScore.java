...
public class RelativenessScore {
    ...
    public Map<String, Double> maxPosition (String[] entityArray, String fromDate, String toDate){
        ...
        for (int i = 1; i < entityArray.length; i++) {
            ParameterizedSparqlString MaxPositionQuery_toAppend = new ParameterizedSparqlString("?article schema:mentions ?entity"+i+" .\n" +
                                                                                                "?entity"+i+" oae:hasMatchedURI <" + entityArray[i] + ">\n");
            MaxPositionQuery.append(MaxPositionQuery_toAppend);
        }

        ParameterizedSparqlString MaxPositionQuery_endAppend = new ParameterizedSparqlString("}");

        MaxPositionQuery.append(MaxPositionQuery_endAppend);
